...
speech_tokenizer_task=gan_speech_tokenizer  # gan_speech_tokenizer or speech_tokenizer
speech_tokenizer_tag=
speech_tokenizer_exp=
speech_tokenizer_stats_dir=
speech_tokenizer_config=
speech_tokenizer_args=
...
Main options:
  ...
  --valid_set NAME
  --test_sets "NAME ..."
  --speech_tokenizer_config PATH
  --speech_tokenizer_task gan_speech_tokenizer|speech_tokenizer
...
then be provided by --speech_tokenizer_config.
EOF
)
...
if [ "${feats_type}" != raw ]; then
    log "Error: speech_tokenizer1 currently supports --feats_type raw only"
    exit 2
fi
if [ "${speech_tokenizer_task}" != gan_speech_tokenizer ] \
        && [ "${speech_tokenizer_task}" != speech_tokenizer ]; then
    log "Error: --speech_tokenizer_task must be gan_speech_tokenizer or speech_tokenizer"
    exit 2
fi
...
if [ -z "${speech_tokenizer_tag}" ]; then
    if [ -n "${speech_tokenizer_config}" ]; then
        speech_tokenizer_tag=${speech_tokenizer_task}_$(basename "${speech_tokenizer_config}" .yaml)_${feats_type}
    else
        speech_tokenizer_tag=${speech_tokenizer_task}_train_${feats_type}
    fi
    if [ "${lang}" != noinfo ]; then
        speech_tokenizer_tag+=_${lang}_${token_type}
    else
        speech_tokenizer_tag+=_${token_type}
    fi
    if [ "${token_type}" = bpe ]; then
        speech_tokenizer_tag+=${nbpe}
    fi
    if [ -n "${speed_perturb_factors}" ]; then
        speech_tokenizer_tag+=_sp
    fi
fi
if [ -z "${speech_tokenizer_stats_dir}" ]; then
    ...
    speech_tokenizer_stats_dir=${expdir}/speech_tokenizer_stats_\
${effective_train_set}_${valid_set}_${feats_type}_${fs}_${audio_format}_\
${stats_token_tag}${stats_speed_tag}_${stats_spemb_tag}
fi
if [ -z "${speech_tokenizer_exp}" ]; then
    speech_tokenizer_exp=${expdir}/speech_tokenizer_${speech_tokenizer_tag}
fi
...
if [ "${stage}" -le 8 ] && [ "${stop_stage}" -ge 8 ] \
    ...
    log "Stage 8: Collect input shapes for ${speech_tokenizer_task}"
    ...
    stats_logdir=${speech_tokenizer_stats_dir}/logdir
    ...
    stats_config_opts=()
    if [ -n "${speech_tokenizer_config}" ]; then
        stats_config_opts+=(--config "${speech_tokenizer_config}")
    fi
    if "${use_default_centroid}"; then
    ...
    ${train_cmd} JOB=1:"${stats_nj}" "${stats_logdir}/stats.JOB.log" \
        "${python}" -m "espnet2.bin.${speech_tokenizer_task}_train" \
            --collect_stats true \
            ...
            "${stats_config_opts[@]}" ${speech_tokenizer_args}
    ...
    "${python}" -m espnet2.bin.aggregate_stats_dirs \
        "${aggregate_opts[@]}" --output_dir "${speech_tokenizer_stats_dir}"
    for split in train valid; do
        awk -v vocab_size="$(wc -l < "${token_list}")" \
            '{print $0 "," vocab_size}' \
            "${speech_tokenizer_stats_dir}/${split}/text_shape" \
            > "${speech_tokenizer_stats_dir}/${split}/text_shape.${token_type}"
    done
fi
...
if [ "${stage}" -le 9 ] && [ "${stop_stage}" -ge 9 ] \
    ...
    train_spemb=${dumpdir}/${spk_embed_tag}/${effective_train_set}/${spk_embed_tag}.scp
    valid_spemb=${dumpdir}/${spk_embed_tag}/${valid_set}/${spk_embed_tag}.scp
    log "Stage 9: Train speech tokenizer: ${speech_tokenizer_exp}"
    mkdir -p "${speech_tokenizer_exp}"

    train_config_opts=()
    if [ -n "${speech_tokenizer_config}" ]; then
        train_config_opts+=(--config "${speech_tokenizer_config}")
    fi
    quantizer_opts=()
    if "${use_default_centroid}"; then
        quantizer_opts+=(--quantizer_conf "centroid_path=${resolved_centroid_path}")
    fi
    jobname=${speech_tokenizer_exp}/train.log
    if echo "${cuda_cmd}" | grep -q -e queue.pl -e queue-freegpu.pl; then
        jobname=$(basename "${speech_tokenizer_exp}")
    fi
    "${python}" -m espnet2.bin.launch \
        --cmd "${cuda_cmd} --name ${jobname}" \
        --log "${speech_tokenizer_exp}/train.log" \
        --ngpu "${ngpu}" \
        --num_nodes "${num_nodes}" \
        --init_file_prefix "${speech_tokenizer_exp}/.dist_init_" \
        ...
        "${python}" -m "espnet2.bin.${speech_tokenizer_task}_train" \
            ...
            --train_data_path_and_name_and_type "${train_spemb},spembs,kaldi_ark" \
            --valid_data_path_and_name_and_type "${valid_spemb},spembs,kaldi_ark" \
            --train_shape_file "${speech_tokenizer_stats_dir}/train/speech_shape" \
            --train_shape_file "${speech_tokenizer_stats_dir}/train/text_shape.${token_type}" \
            --valid_shape_file "${speech_tokenizer_stats_dir}/valid/speech_shape" \
            --valid_shape_file "${speech_tokenizer_stats_dir}/valid/text_shape.${token_type}" \
            --train_shape_file "${speech_tokenizer_stats_dir}/train/spembs_shape" \
            --valid_shape_file "${speech_tokenizer_stats_dir}/valid/spembs_shape" \
            --output_dir "${speech_tokenizer_exp}" \
            "${train_config_opts[@]}" ${speech_tokenizer_args}
fi
...
if [ "${stage}" -le 10 ] && [ "${stop_stage}" -ge 10 ] \
    ...
    for dset in ${test_sets}; do
        ...
        decode_dir=${speech_tokenizer_exp}/${inference_tag}/${dset}
        ...
        ${inference_cmd} --gpu "${inference_ngpu}" JOB=1:"${decode_nj}" \
            ...
            "${python}" -m espnet2.bin.speech_tokenizer_inference \
                --ngpu "${inference_ngpu}" \
                --batch_size "${inference_batch_size}" \
                --data_path_and_name_and_type "${decode_data}/wav.scp,speech,sound" \
                --key_file "${decode_logdir}/keys.JOB.scp" \
                --train_config "${speech_tokenizer_exp}/config.yaml" \
                --model_file "${speech_tokenizer_exp}/${inference_model}" \
