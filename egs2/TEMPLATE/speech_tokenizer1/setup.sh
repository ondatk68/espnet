...
for item in cmd.sh conf local; do
    cp -r "${dir}/../../TEMPLATE/speech_tokenizer1/${item}" "${dir}"
done

# These files are maintained centrally by the template.
for item in speech_tokenizer.sh path.sh db.sh scripts pyscripts steps utils; do
    ln -sf "../../TEMPLATE/speech_tokenizer1/${item}" "${dir}/${item}"
done
